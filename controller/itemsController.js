...
// TODO #1.2: Add an item to DynamoDB    //ต้องส่งข้อมูล (post)
exports.addItem = async (req, res) => {
  
  const item_id = uuidv4();
  const created_date = Date.now();
  const item = { item_id: item_id, ...req.body, created_date: created_date };
  // ...req.body is recieve data from body of request
  // You should change the response below.
  // ---provided---
  ...
  try{
    ...
    res.send(data)  //you need to response
  }
  catch (err){
    console.error(err);
    res.status(500).send(err);
...
exports.deleteItem = async (req, res) => {
  const item_id = req.params.item_id;           //item_id of item we want to delete
